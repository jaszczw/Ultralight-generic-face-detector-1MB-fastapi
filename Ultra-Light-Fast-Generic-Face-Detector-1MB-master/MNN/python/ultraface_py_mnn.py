...
sys.path.append(os.path.abspath('./Ultra-Light-Fast-Generic-Face-Detector-1MB-master'))
import vision.utils.box_utils_numpy as box_utils

dir_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# parser = argparse.ArgumentParser(description='run ultraface with MNN in py')
# parser.add_argument('--model_path', default= dir_path + "/MNN/model/version-RFB/RFB-320.mnn", type=str, help='model path')
...
# parser.add_argument('--imgs_path', default= dir_path + "/MNN/imgs", type=str, help='imgs dir')
# parser.add_argument('--results_path', default= dir_path + "/results", type=str, help='results dir')
# args = parser.parse_args()

input_size = (320,240)
model_path = dir_path + "/MNN/model/version-RFB/RFB-320.mnn"
threshold = 0.7
imgs_path = dir_path + "/MNN/imgs"
...
def predict(width, height, confidences, boxes, prob_threshold, iou_threshold=0.3, top_k=-1):
    ...
    picked_box_probs = []
    picked_labels = []
    print(width,height,prob_threshold)
...
def inference(image_input):
    # input_size = [int(v.strip()) for v in input_size.split(",")]
